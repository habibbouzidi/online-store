                ...
                <div className={classNames.productInfo}>
                    <div className={classNames.cartContainer}>
                        <button onClick={handleDecrease}
                            className={`${classNames.button} ${quantity === 0 && classNames.disabledButton}`} disabled={quantity === 0}>
                            -</button>
                        <input type="text" className={classNames.quantityInput} value={quantity} readOnly />
                        {game &&
                            <button onClick={handleIncrease}
                                className={`${classNames.button} ${quantity === game.quantity && classNames.disabledButton}`} disabled={quantity === game.quantity}>
                                +</button>
                        }
                        <div className={classNames.cartPrice}>$ {price}</div>
                        <button className={`${classNames.button} ${quantity === 0 && classNames.disabledButton}`} disabled={quantity === 0}>
                            <FaShoppingCart />
                        </button>
                    </div>

                    {/*Render game details server side*/}
                    {children}
                </div>
            </div>
        </div>
